...
- **WHEN** `kubectl` is on `PATH`
- **THEN** the command uses `kubectl` to perform the operation

#### Scenario: Only docker available
- **WHEN** `kubectl` is not on `PATH` but `docker` is
- **THEN** the command uses `docker-compose` to perform the operation

#### Scenario: No runtime available
- **WHEN** neither `kubectl` nor `docker` is on `PATH`
- **THEN** the command prints `'Error: cannot find required binaries.'` and exits non-zero
...
- **WHEN** `nita-cmd_jenkins_up` is invoked with docker available
...
- **THEN** the Jenkins compose stack is stopped and containers removed
...
- **THEN** the Jenkins container is stopped but its named volume and data are retained
...
- **WHEN** `nita-cmd_jenkins_jobs_create myjob` is called and `myjob.xml` exists
...
`nita-cmd_jenkins_cli_jenkins` and `nita-cmd_jenkins_cli_root` SHALL open an interactive Jenkins CLI session as the `jenkins` user or as `root` respectively.

#### Scenario: CLI opens as jenkins user
- **WHEN** `nita-cmd_jenkins_cli_jenkins` is called
- **THEN** an interactive shell or CLI session is opened inside the container as the `jenkins` user
...
- **WHEN** `_CLI_RUNNER_DEBUG=1 nita-cmd_jenkins_up` is called
...
- **WHEN** `NITAJENKINSDIR=/home/jcluser/nita-jenkins nita-cmd_jenkins_up` is called
...
- **WHEN** the `cli_scripts/` directory is inspected
